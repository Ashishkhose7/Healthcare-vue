...
<template>
  <div class="diagnosis-history">
    <div class="chart-info">
      ...
      <div
        class="chart flex justify-between gap-2 xl:gap-6 p-2 rounded-lg h-[250px]"
      >
        <div class="chart-sectio flex flex-col gap-2 w-[60%]">
          ...
          <div
            class="line-chart h-[250px]"
            v-if="userStore.getactiveUsers.diagnosis_history"
          >
        ...
        <div
          v-if="userStore.getactiveUsers.diagnosis_history"
          class="chart-information bg-transparent px-2 w-[40%]"
        >
          <span
            class="circle inline-block h-[10px] w-[10px] rounded-full bg-pink-500 text-xs"
          ></span
          ><span class="text-base mx-1 font-semibold my-1">Systolic</span>
          <br />
          <span
            v-if="userStore?.getactiveUsers?.diagnosis_history"
            class="font-semibold text-xl sysvalue inline-block my-1"
            >{{
              userStore?.getactiveUsers?.diagnosis_history[0]?.blood_pressure
                ?.systolic?.value
            }}</span
          ><br />
          <svg
          ...
          <span class="text-base text-slate-600 font-normal syslevel ml-1">{{
            userStore?.getactiveUsers?.diagnosis_history[0]?.blood_pressure
              ?.systolic?.levels
          }}</span>
          <hr class="my-3" />
          <span
            class="circle inline-block h-[10px] w-[10px] rounded-full bg-purple-500 text-xs"
          ></span
          ><span class="text-base mx-1 font-semibold">Diastolic</span>
          <br />
          <span
            v-if="userStore?.getactiveUsers?.diagnosis_history"
            class="font-semibold text-xl diavalue inline-block my-2"
            >{{
              userStore?.getactiveUsers?.diagnosis_history[0]?.blood_pressure
                ?.diastolic?.value
            }}</span
          ...
          <span class="text-base text-slate-600 font-normal dialevel ml-1">{{
            userStore?.getactiveUsers?.diagnosis_history[0]?.blood_pressure
              ?.diastolic?.levels
          }}</span>
        </div>
      ...
      <div class="vitals" v-if="userStore.getactiveUsers.diagnosis_history">
        <div class="vital-item respiratory">
          <img
            src="../assets/img/respiratory rate.png"
            alt=""
            class="h-[60px]"
          />
          ...
          <p
            v-if="userStore?.getactiveUsers?.diagnosis_history"
            class="font-bold text-xl resrate"
          >
            {{
              userStore?.getactiveUsers?.diagnosis_history[0]?.respiratory_rate
                ?.value
            }}
            bpm
          </p>
          <p class="mt-2 vital-item-level reslevel">
            {{
              userStore?.getactiveUsers?.diagnosis_history[0]?.respiratory_rate
                ?.levels
            }}
        ...
        <div class="vital-item temperature">
          ...
          <p
            v-if="userStore?.getactiveUsers?.diagnosis_history"
            class="font-bold text-xl temp"
          >
            {{
              userStore?.getactiveUsers?.diagnosis_history[0]?.temperature
                ?.value
            }}°F
          </p>
          <p class="mt-2 vital-item-level templevel">
            {{
              userStore?.getactiveUsers?.diagnosis_history[0]?.temperature
                ?.levels
            }}
        ...
        <div class="vital-item heart-rate">
          ...
          <p
            v-if="userStore?.getactiveUsers?.diagnosis_history"
            class="font-bold text-xl heartrate"
          >
            {{
              userStore?.getactiveUsers?.diagnosis_history[0]?.heart_rate?.value
            }}
            bpm
          </p>
          <svg
            v-if="
              userStore?.getactiveUsers?.diagnosis_history[0]?.heart_rate
                ?.levels !== 'Normal'
            "
            class="inline-block"
            xmlns="http://www.w3.org/2000/svg"
